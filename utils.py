...
def markdown_css(
    ...
    """
    css_txt = (
        f'<p style="font-family:sans-serif;color:{color}; font-size: {font_size}px;'
...
def click_button(
    txt: str, on_click=None, args=None, size: int = 20, position: int = None
):
    """
    ...
    if position is None:
        css_style = (
            f"<style>div.stButton > button:first-child"
        ...
        css_style = (
            f"<style>div.stButton > button:first-child {{background-color:{color};"
