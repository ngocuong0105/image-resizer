...
def open_file() -> "tuple[ResizeableImage,str]":
    ...
    uploaded_file = st.file_uploader(
        "Upload your own image file", type=["png", "jpg", "jpeg"]
    )
...
def run() -> None:
    ...
    if mode == options[0]:
        ...
        options = [
            "🗜️ Simply compress image",
            "🛡️ Compress image with protected area",
            "✂️ Compress image with object removal",
        ]
        ...
        if compress_mode == d["simple"]:
            ...
            resImage.protected = canvas_protection(
                resImage, tool_txt, start_txt, end_txt, resImage.height, resImage.width
            )
            ...
            end_txt = "You are ready to compress!"
            resImage.removed = canvas_protection(
                resImage, tool_txt, start_txt, end_txt, resImage.height, resImage.width
            )
        ...
        options = [
            "🖼️ Simply enlarge image",
            "🛡️ Enlarge with protected area",
            "✏️ Enlarge with specified area for enlargement",
        ]
        d = dict(zip(["simple", "protect", "removal"], options))
        ...
        st.write(
            "Softening certain parts of the image is usually used after object \
...
def compress_image(
    resImage: resizeableImage, new_width: int, remove: bool = False
) -> None:
    """
    ...
    if not remove:
        ...
        for i in range(w - new_width):
            ...
            resImage.color_seam(
                vertical_seam, st.session_state.get("default_seam", [0, 0, 255])
            )
            img_place.image(
                resImage.encodeBytes(resImage.format_type), width=resImage.width
            )
...
def enlarge_image(resImage: resizeableImage, new_width: int) -> None:
    ...
    for i in range(new_width - w):
        ...
        image_place.image(
            resImage_copy.encodeBytes(resImage.format_type), width=resImage.width
        )
...
def new_width_height(resImage: ResizeableImage) -> tuple:
    ...
    if width_removed < 0 or height_removed < 0:
        st.write(
            "Please select width and height which are smaller than the initial ones."
        )
        st.stop()
...
def new_width_height_enlarge(resImage: ResizeableImage) -> tuple:
    ...
    if width_removed > 0 or height_removed > 0:
        st.write(
            "Please select width and height which are larger than the initial ones."
        )
        st.stop()
    return new_height, new_width


def canvas_protection(
    resImage: ResizeableImage,
    tool_txt: str,
    start_txt: str,
    end_txt: str,
    height: float,
    ...
    if drawing_mode == "rectangle":
        ...
        st.write(
            "• Polygon should have more than 4 vertices and concave shapes are discouraged."
        )
    # Create a canvas
    ...
    if canvas_result.json_data and canvas_result.json_data["objects"] != []:
        ...
        if drawing_mode == "rect":
            ...
            center = int(canvas_dict["top"] + canvas_dict["radius"]), int(
                canvas_dict["left"] + canvas_dict["radius"]
            )
