...
class ResizeableImage:
    ...
    def energy(self, i: int, j: int) -> float:
        ...
        if (
            ...
            return (
                self.distance(self.pixels[i][j - 1], self.pixels[i][j + 1])
    ...
    def scharr_energy_mat(self) -> "np.array[float]":
        ...
        b_energy = np.square(cv2.Scharr(b, -1, 1, 0)) + np.square(
            cv2.Scharr(b, -1, 0, 1)
        )
        g_energy = np.square(cv2.Scharr(g, -1, 1, 0)) + np.square(
            cv2.Scharr(g, -1, 0, 1)
        )
        r_energy = np.square(cv2.Scharr(r, -1, 1, 0)) + np.square(
            cv2.Scharr(r, -1, 0, 1)
        )
